  ...
  test.describe(``, { tag: [TAGS.DOC.DOCUMENTED, TAGS.TYPE.WEB, TAGS.STATUS.EXPECTED_TO_PASS] }, () => {
    ...
    test(`Scenario_1`, async ({ pages, testData }) => {
      ...
      await test.step(`Add all=${invetoryPageItemsAmount} inventory items to cart`, async () => {
        ...
        expect(inventoryAllItemsAZ).toHaveLength(invetoryPageItemsAmount);
      ...
      await test.step(`Validate initial cart content and item count`, async () => {
        ...
        expect(cartAllItemsBeforeAZ).toHaveLength(invetoryPageItemsAmount);
        expect(cartAllItemsBeforeAZ.map(({ quantity: _, ...rest }) => rest)).toEqual(inventoryAllItemsAZ);
        await expect(pages.cart.header.headerPrimary.shoppingBadge).toHaveText(invetoryPageItemsAmount.toString());
      });
      ...
      await test.step(`Validate cart content and item count after item removal`, async () => {
        ...
        expect(cartAllItemsAfterAZ).toHaveLength(invetoryPageItemsAmount - 1);
        await expect(pages.cart.header.headerPrimary.shoppingBadge).toHaveText((invetoryPageItemsAmount - 1).toString());
        await expect(pages.cart.items.itemByName(itemRemoveName).name).toBeHidden();
      });
    ...
    test(`Scenario_2 - with standard_user and random item name`, async ({ pages }) => {
      ...
      await test.step(`Validate item details page`, async () => {
        ...
        const { id: _, ...expecdetDetails } = randomItem;
        const itemDetails = {
      ...
      await test.step(`Validate cart content`, async () => {
        ...
        expect(cartItems).toHaveLength(cartItemsAmount);
        await expect(pages.cart.header.headerPrimary.shoppingBadge).toHaveText(cartItemsAmount.toString());
        const { quantity: _, ...cartItem } = cartItems[0];
