...
- Name: api/api-automation.spec.ts >> API Automation tests >> Scenario_4 with original delayMs=6000
...
```ts
  31  |         });
  32  | 
  33  |         await test.step(`Extract and attach titles of products with odd IDs`, async () => {
  34  |           const filteredTitles = productsList.filter(p => (p.id as number) % 2 !== 0).map(p => `id=${p.id}: title=${p.title}`);
  35  |           await test.info().attach('filtered_odd_titles', { body: JSON.stringify(filteredTitles) });
  36  |         });
  37  |       });
  38  | 
